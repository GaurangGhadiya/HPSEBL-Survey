...
const checkCookies = (context) => {
    const userName = context.req
        ? Cookies.get('hpsebl_userName', { headers: context.req.headers })
        : Cookies.get('hpsebl_userName');
    const ulb = context.req ? Cookies.get('hpsebl_ulb', { headers: context.req.headers }) : Cookies.get('hpsebl_ulb');
    return { userName, ulb };
};
